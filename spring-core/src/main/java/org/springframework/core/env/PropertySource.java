 ...
 * implementations.
 *
 * 表示名称/值属性对的源的抽象基类。 底层{@linkplain #getSource（）源对象}可以是封装的任何类型{@code T}
 * 属性。 示例包括{@link java.util.Properties}对象，{@link java.util.Map}对象，{@code ServletContext}和{@code ServletConfig}对象（用于访问init参数）。 浏览{@code PropertySource}类型层次结构以查看提供的内容
 * 实现。
 *
