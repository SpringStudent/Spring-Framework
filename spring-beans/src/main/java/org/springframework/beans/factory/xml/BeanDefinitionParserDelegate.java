...
/**
 * 用于解析xml到bean defintions的有状态的代理类
 *旨在供主解析器和任何扩展使用
...
public class BeanDefinitionParserDelegate {
	...
	/**
	 * 解析提供的{@code <bean>}元素。 如果在解析期间出现错误，可能会返回{@code null}。 报告错误
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, BeanDefinition containingBean) {
		...
		if (beanDefinition != null) {
			//如果未定义beanName，根据Spring提供命名规则为当前bean生成对应的name
			if (!StringUtils.hasText(beanName)) {
				try {
					if (containingBean != null) {
						...
						//有可能的话，注册bean class name到alias，如果生成器返回类名加后缀
	 ...
	 *
	 * 验证指定的bean名称和别名尚未在当前<beans>元素嵌套级别中使用。
	 */
	 ...
	 *
	 *将bean标签的指定属性赋值给定的bean definition
	...
	/**
	 * 使用给定的className和parentName属性创建BeanDefintion对象
	...
	/**
	 * 获取属性元素的值。 可能是列表等。也用于构造函数参数，“propertyName”在这种情况下为null
	 */
