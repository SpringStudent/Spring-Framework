...
/**
 * 作为类似{@link org.springframework.transaction.jta.JtaTransactionManager}具体平台事务管理器的抽象
 * 的实现了Spring标准工作事务流的抽象基类
 *
 * <p>此基类提供以下工作流处理：
 * <ul>
 * <li>确定是否存在事务
 ...
 *
 * <p>子类必须实现特定的特定模板方法
 *   事务的状态，例如：begin，suspend，resume，commit，rollback。
 *  子类最重要 的事情就是对抽象方法提供具体实现;其余部分，已经提供了默认实现
 *  因此覆盖方法不是必须的
 *
 * 事务同步是一种用于在事务完成时注册回调的调用机制。当在JTA事务中运行时，
 * 这主要由JDBC，Hibernate，JPA等的数据访问支持类在内部使用：他们注册在事务完成时关闭的在一
 * 个事务中的资源。比如在一个事务中的Hibernate Session.同样的机制也可以用于应用程序中的自定义同步需求。
 *
 * 此类的是可序列化的，以允许序列化事务策略以及携带事务拦截器的代理.
 * 如果他们希望使其状态也可序列化，则由子类决定。
 * 在这种情况下，他们应该实现{@code java.io.Serializable}标记接口，
 * 如果需要恢复任何瞬态，可能会使用私有的{@code readObject（）}方法（根据Java序列化规则）
 *
...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	 ...
	 * 总是激活事务同步,甚至是由PROPAGATION_SUPPORTS产生的“空”事务，没有现有的后端交易
	 ...
	 * as rollback-only.
	 *
	 * 在事务被全局标记为rollback-only的情况下返回是否提前失败
	 * @since 2.0
	 */
	...
	public final TransactionStatus getTransaction(TransactionDefinition definition) throws TransactionException {
		//由子类实现;获取transaction对象
		Object transaction = doGetTransaction();

		// Cache debug flag to avoid repeated checks.
		boolean debugEnabled = logger.isDebugEnabled();
		//如果事务属性未设置，则使用默认的事务属性
		if (definition == null) {
			// Use defaults if no transaction definition given.
			definition = new DefaultTransactionDefinition();
		}
		//判断当前线程是否存在事务;存在的话，处理子事务
		...
		//上文已经判断了当前是否存在事务;如果不存在事务;使用当前事物，如果当前没有事物，则抛出异常
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			throw new IllegalTransactionStateException(
					"No existing transaction found for transaction marked with propagation 'mandatory'");
		}
		//没有子事务；并且事务传播特性需要创建新事务
			...
			try {
				...
				//构造transaction,设置connectionHolder、隔离级别、timeout
	...
	private TransactionStatus handleExistingTransaction(
		...
		if (isValidateExistingTransaction()) {
			...
			if (definition.getIsolationLevel() != TransactionDefinition.ISOLATION_DEFAULT) {
				...
				//如果当前事务隔离级别为null或者不和definition的隔离级别一致 抛出异常
	...
	protected void prepareSynchronization(DefaultTransactionStatus status, TransactionDefinition definition) {
		if (status.isNewSynchronization()) {
			...
			//初始化synchronizations 就是绑定 new LinkedHashSet<TransactionSynchronization>()给当前线程
	...
	protected final SuspendedResourcesHolder suspend(Object transaction) throws TransactionException {
		//TODO 扩展点 当前线程事务同步属于活跃状态 我们可以通过方法
		// TransactionSynchronizationManager.registerSynchronization注册事务同步器
		// 这样我们便可以在事务执行过程中做一些自定义的骚操作
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			List<TransactionSynchronization> suspendedSynchronizations = doSuspendSynchronization();
			try {
				Object suspendedResources = null;
				if (transaction != null) {
					//挂起事务 线程与connectionHolder绑定被解除了，并返回了connectionHolder
					//用于下次执行事务使用！！！
	...
	private void triggerAfterCompletion(DefaultTransactionStatus status, int completionStatus) {
		if (status.isNewSynchronization()) {
			...
			if (!status.hasTransaction() || status.isNewTransaction()) {
				...
				//调用TransactionSynchronization 的 afterCompletetion
				invokeAfterCompletion(synchronizations, completionStatus);
			}
			else if (!synchronizations.isEmpty()) {
				//如果是参与已经存在的事务
				//在Spring事务管理器范围之外控制 - >尝试使用现有（JTA）事务注册afterCompletion回调
				//不是JTA事务 处理方式和invokeAfterCompletion相同
	 ...
	 *
	 * <p>当事务管理器决定实际启动新事务时，将调用此方法。 之前没有任何事务，或者之前的事务已被暂停。
	 * <p>特殊情况是没有保存点的嵌套事务：If
	 * {@code useSavepointForNestedTransaction（）}返回“false”，将调用此方法以在必要时启动嵌套事务。
	 * 在这样的上下文中，将存在活动事务：此方法的实现必须检测并启动适当的嵌套事务。
